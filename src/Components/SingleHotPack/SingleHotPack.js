...
const SingleHotPack = () => {
    ...
    // const [allPacks, setAllPack] = useState([]);
    ...
    useEffect(() => {
        fetch(`https://floating-plains-57094.herokuapp.com/addHotPackage/${hpid}`)
            .then(res => res.json())
            .then(data => setSinglePack(data))
    }, [hpid]);

    // useEffect(() => {
    //     const findSingle = allPacks.find(allPack => allPack?._id.toString() === hpid);

    //     setSinglePack(findSingle)
    ...
    return (
        <div className='container pack'>
            <div className="tour_container">
                <h1 className='h1'>{singlePack?.hpname}</h1>
                <div className="icon">
                    ...
                    <h3 style={{ color: "#4C4646", fontWeight: "bolder" }}>{singlePack?.hplocation}</h3>
                </div>
                <div className="tour_details">
                    <div className="duration">
                        ...
                        <span style={{ fontSize: "18px", fontWeight: "bolder", color: "#0869D0 " }}>{singlePack?.hpduration}</span>
                    </div>
                    <div className="tour_type">
                        <svg xmlns="http://www.w3.org/2000/svg" style={{ width: "40px", color: "#5ED3F9" }} className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        <span style={{ fontSize: "18px", fontWeight: "bolder", color: "#0869D0 " }}>{singlePack?.hptourType}</span>
                    </div>
                    <div className="tour_type">
                        ...
                        <span style={{ fontSize: "22px", fontWeight: "bolder", color: "#0869D0 " }}>{singlePack?.hpmaxGroupSize}</span>
                    </div>
                </div>
                <img src={singlePack?.hpimageCard} alt="" />
                <div className="food p-5">
                    <h1>Food Menu</h1>
                    <h3>{singlePack?.hpfoodMenu}</h3>
                </div>
                <div className="">
                    <h1>Includes</h1>
            ...
            <div className="book_container">
                <div className="form">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        />
                        <input
                            defaultValue={singlePack?.hpname}
                            placeholder=''
                            {...register("pacName", { required: true })}
                        />

                        <input
                            defaultValue={singlePack?.hptourType}
                            {...register("packageType", { required: true })}
                        />
                        <input

                            defaultValue={singlePack?.hpdate}
                            {...register("date", { required: true })}
                        />
                        <input
                            defaultValue={singlePack?.hpcost}
                            {...register("amount", { required: true })}
                        />

                        {user.email ? < input type="submit" value="Book Package" />
                            : <Link to='/login'>
                                <Button >Book To Login</Button>
                            </Link>
                        }
                    </form>
                </div>
